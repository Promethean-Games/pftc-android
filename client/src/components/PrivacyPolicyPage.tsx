...
const ALL_LOCALSTORAGE_KEYS = [
  "currentGame", "savedGames", "pftc_unlocked", "appScreen", "appActiveTab",
  "appViewOnly", "setupTimes", "pftc_leveler_tutorial_seen", "pftc_level_home",
  "pftc_level_league", "pftc_level_tournament", "pftc_terms_accepted",
  "pftc_terms_version", "pftc_terms_accepted_at",
  "pftc_analytics_opt_out", "sidebar:state",
];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-2xl mx-auto p-4 pb-16">
        <div className="flex items-center justify-between gap-2 mb-4 sticky top-0 bg-background py-3 z-10 border-b">
          <h1 className="text-xl font-bold">Terms of Use &amp; Privacy Policy</h1>
          <Button size="icon" variant="ghost" onClick={() => window.print()} data-testid="button-print-policy">
            <Printer className="w-4 h-4" />
          </Button>
        </div>

        <p className="text-xs text-muted-foreground mb-6">Promethean Games LLC &middot; Effective Mar 17, 2026</p>
        ...
        <div className="space-y-4 text-sm leading-relaxed">
          ...
          <Card className="p-4">
            ...
            <ul className="text-muted-foreground list-disc pl-4 space-y-2">
              ...
              <li><strong>Anonymous usage analytics</strong>: We collect anonymous, aggregate analytics events to help us improve the App, which are forwarded to <a href="https://posthog.com/privacy" target="_blank" rel="noopener noreferrer" className="underline">PostHog</a> as a third-party analytics processor. Events are limited to: app opened (platform, app version), game started (number of players, demo vs full), game completed (holes played, approximate duration), paywall encountered, purchase initiated or completed, and which CueMaster Tools are opened. Each browser session is assigned a temporary anonymous ID stored in session storage (automatically cleared when you close the browser tab or app) — no persistent identifier is ever stored on your device or sent to any server. <strong>We do not collect</strong> player names, individual scores, IP addresses, or any personally identifiable information. No cookies are used. You can opt out at any time in Settings. <em>For Google Play Data safety purposes: this constitutes "App activity — App interactions" shared with PostHog for Analytics.</em></li>
              <li><strong>Payment processing</strong>: On the web, purchases are handled by Stripe, which collects your payment information directly — we do not store or have access to your payment details. See <a href="https://stripe.com/privacy" target="_blank" rel="noopener noreferrer" className="underline">Stripe's Privacy Policy</a>. On Android via Google Play, a one-time purchase token is transmitted to our server solely to verify your purchase with Google; it is used ephemerally and not retained.</li>
          ...
          <Card className="p-4">
            ...
            <p className="text-muted-foreground mt-2">Player names, individual scores, and other gameplay data never leave your device. Anonymous analytics events (described in Section 6) are forwarded to PostHog using a temporary session ID in session storage — this ID is automatically cleared when you close the browser tab or app, and no persistent identifier is stored on your device.</p>
          </Card>
          ...
          <Card className="p-4">
            ...
            {!deleted ? (
              !showDeleteConfirm ? (
                <Button variant="destructive" onClick={() => setShowDeleteConfirm(true)} data-testid="button-delete-data">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" onClick={() => setShowDeleteConfirm(false)} data-testid="button-cancel-delete">Cancel</Button>
                    <Button variant="destructive" onClick={handleDeleteAllData} data-testid="button-confirm-delete">Confirm Delete</Button>
                  </div>
                </div>
              )
            ) : (
